...
class ObsidianFormatter:
    ...
    def __init__(
        self,
        remove_top_level_bullets=False,
        category_tag=None,
                ...
                for match in re.finditer(highlight_pattern, content, re.DOTALL):
                    ...
                    if screenshot_dir.exists():
                        ...
                        if screenshot_files:
                            # 使用相对路径
                            screenshot_path = (
                                f"attachments/{pdf_name}/{screenshot_files[0].name}"
                            )
    ...
    def format_content(
        self, parsed_data: Dict, filename: str = "", target_folder: str = ""
    ) -> str:
        ...
        frontmatter = self._generate_frontmatter(
            parsed_data.get("meta_properties", []), filename
        )

        # 移除原始内容中的 meta 属性行
        filtered_lines = self._filter_meta_lines(
            lines, parsed_data.get("meta_properties", [])
        )
        ...
        # 格式优化：处理空行和标题间距
        formatted_lines = self._optimize_formatting(
            formatted_lines
        )  # 组合 frontmatter 和内容
        if frontmatter:
    ...
    def _convert_video_embeds(self, line: str) -> str:
        ...
        processed_line = re.sub(
            r"\{\{video\s+([^\}]+)\}\}", replace_video, processed_line
        )

        # 2. YouTube 嵌入 {{youtube VIDEO_ID或URL}}
        processed_line = re.sub(
            r"\{\{youtube\s+([^\}]+)\}\}", replace_youtube, processed_line
        )

        # 3. Bilibili 嵌入 {{bilibili BV_ID}}
        processed_line = re.sub(
            r"\{\{bilibili\s+([^\}]+)\}\}", replace_bilibili, processed_line
        )
                ...
                if page:
                    ...
                    if pdf_path.startswith("../assets/"):
                        if target_folder:
                            ...
                            converted_pdf_path = pdf_path.replace(
                                "../assets/", "../attachments/"
                            )
                        else:
                    ...
                    if converted_pdf_path.startswith("../attachments/"):
                        clean_pdf_path = converted_pdf_path.replace(
                            "../attachments/", ""
                        )
                    ...
                    if coordinates:
                        ...
                        if "width" in coordinates and "height" in coordinates:
                            ...
                            selection = (
                                f"{x1_scaled},{y1_scaled},{x2_scaled},{y2_scaled}"
                            )
    ...
    def get_conversion_summary(
        self, original_data: Dict, converted_content: str
    ) -> Dict:
        """生成转换摘要"""
        original_stats = {
            "page_links": len(
                re.findall(r"\[\[([^\]]+)\]\]", original_data["content"])
            ),
            "block_refs": len(re.findall(r"\(\(([^)]+)\)\)", original_data["content"])),
            "block_ids": len(
                re.findall(
                    r"^\s*id:: ([a-zA-Z0-9-]+)", original_data["content"], re.MULTILINE
                )
            ),
            "assets": len(
                re.findall(r"!\[([^\]]*)\]\(([^)]+)\)", original_data["content"])
    ...
    def _generate_frontmatter(
        self, meta_properties, original_filename: str = ""
    ) -> str:
        """生成 YAML frontmatter"""
        frontmatter_lines = ["---"]
    ...
    def detect_category_folder(self, parsed_data: Dict) -> str:
        ...
        if match:
            if is_quote_block:
                ...
                if not content_before_tag or re.match(
                    r"^(\s*#\w+\s*)+$", content_before_tag
                ):
                    return self.category_folder
    ...
    def _get_actual_content_lines(self, lines, meta_properties):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if meta_content_patterns and any(
                pattern in line_content for pattern in meta_content_patterns
            ):
                continue
        ...
        for line in lines:
            ...
            if category_tag_pattern in content_without_bullets:
                ...
                if content_without_bullets.strip() == category_tag_pattern:
                    ...
                    # 标签不在开头，替换标签部分
                    new_content = content_without_bullets.replace(
