...
class FileManager:
    ...
    def copy_assets(
        self, source_paths: List[Path], target_subdir: str = "attachments"
    ) -> List[Path]:
        """复制资源文件"""
        ...
        for i, conversion in enumerate(conversions, 1):
            ...
            if "summary" in conversion:
                ...
                report_lines.extend(
                    [
                        "**转换统计:**",
                        f"- 页面链接: {summary.get('original', {}).get('page_links', 0)} "
                        f"-> {summary.get('converted', {}).get('page_links', 0)}",
                        f"- 块引用: {summary.get('original', {}).get('block_refs', 0)} "
                        f"-> {summary.get('converted', {}).get('block_refs', 0)} (转为注释)",
                        f"- 块ID: {summary.get('original', {}).get('block_ids', 0)} "
                        f"-> {summary.get('converted', {}).get('block_ids', 0)}",
                        f"- 资源文件: {summary.get('original', {}).get('assets', 0)} "
                        f"-> {summary.get('converted', {}).get('assets', 0)}",
                        "",
                    ]
                )

            if "error" in conversion:
                report_lines.extend([f"**错误信息:** {conversion['error']}", ""])
