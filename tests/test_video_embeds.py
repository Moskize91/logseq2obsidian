...
class TestVideoEmbeds(unittest.TestCase):
    ...
    def test_youtube_embed_basic(self):
        ...
        test_cases = [
            ...
            ("这里有视频: {{youtube dQw4w9WgXcQ}}", "行内YouTube嵌入",
             '这里有视频: ![](https://youtu.be/dQw4w9WgXcQ)'),
            ("- {{youtube dQw4w9WgXcQ}}", "列表中的YouTube嵌入",
             '- ![](https://youtu.be/dQw4w9WgXcQ)'),
        ]
        ...
        for input_text, description, expected_output in test_cases:
            with self.subTest(description=description):
                ...
                self.assertEqual(result, expected_output, f"转换结果应该匹配期望输出: {description}")
    
    def test_bilibili_embed_basic(self):
        """测试Bilibili嵌入语法"""
        test_cases = [
            ("{{bilibili BV1xx411c7mD}}", "Bilibili BV号",
             "[Bilibili视频: BV1xx411c7mD](BV1xx411c7mD)"),
            ("观看视频: {{bilibili BV1xx411c7mD}}", "行内Bilibili嵌入",
    ...
    def test_youtube_url_extraction(self):
        ...
        test_cases = [
            ("{{youtube https://youtu.be/rEOoItpzlVw}}", "youtu.be完整URL",
             '![](https://youtu.be/rEOoItpzlVw)'),
            ("{{youtube https://www.youtube.com/watch?v=dQw4w9WgXcQ}}", "完整YouTube URL",
             '![](https://www.youtube.com/watch?v=dQw4w9WgXcQ)'),
            ("{{video https://youtu.be/Urid8hF54_k}}", "通用video标签",
             '![](https://youtu.be/Urid8hF54_k)'),
            ("{{youtube abc123}}", "纯视频ID转为youtu.be格式",
        ...
        for input_text, description, expected_output in test_cases:
            with self.subTest(description=description):
                ...
                print(f"输出: {result}")
                print(f"期望: {expected_output}")
                self.assertEqual(result, expected_output, f"URL处理转换结果应该匹配期望输出: {description}")

    def test_youtube_timestamp_embed(self):
        """测试YouTube时间戳嵌入语法 - 应该保持原样"""
        test_cases = [
            ("{{youtube-timestamp 8:40}}", "时间戳格式",
             "{{youtube-timestamp 8:40}}"),
            ("{{youtube-timestamp 1:23:45}}", "长时间戳格式",
             "{{youtube-timestamp 1:23:45}}"),
            ("从这里开始: {{youtube-timestamp 8:40}}", "行内时间戳",
    ...
    def test_video_embed_edge_cases(self):
        ...
        test_cases = [
            # 空格处理
            ("{{youtube  dQw4w9WgXcQ  }}", "带额外空格的YouTube", "![](https://youtu.be/dQw4w9WgXcQ)"),
            ("{{bilibili  BV1xx411c7mD  }}", "带额外空格的Bilibili", "[Bilibili视频: BV1xx411c7mD](BV1xx411c7mD)"),
            ("{{youtube-timestamp  8:40  }}", "带额外空格的时间戳", "{{youtube-timestamp  8:40  }}"),
            
            # 多个视频在一行
            ("{{youtube abc123}} 和 {{bilibili BV456}}", "一行多个视频", 
             "![](https://youtu.be/abc123) 和 [Bilibili视频: BV456](BV456)"),
            
            # 混合其他语法
            ("[[页面链接]] {{youtube abc123}} ((block-ref))", "混合语法",
