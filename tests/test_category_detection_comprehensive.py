...
- 使用 [[Kademlia]] 的分布式公开文件系统，存储文件全量。有点类似 [[BitTorrent]]。"""
        ...
        ipfs_data = {"content": ipfs_content, "meta_properties": []}
...
- CRDT 首先要解决排序问题：让不同客户端的操作经过无中心的广播后，在所有端能够收敛到相同的顺序，从而保证执行这些操作后能够到达一样的结果."""
        ...
        crdt_data = {"content": crdt_content, "meta_properties": []}
    ...
    def test_various_tag_formats(self):
        ...
        test_cases = [
            ...
            {"name": "直接标签", "content": "- #wiki 内容\n- 其他内容", "expected": "Wiki"},
            {"name": "标签在行首（无列表标记）", "content": "#wiki 内容\n- 其他内容", "expected": "Wiki"},
            {"name": "标签不在第一行", "content": "- 其他内容\n- #wiki 内容", "expected": ""},
            {"name": "有空行的情况", "content": "\n\n- #wiki 内容\n- 其他内容", "expected": "Wiki"},
            {
                "name": "有meta属性的情况",
                "content": "alias:: 别名\ntitle:: 标题\n\n- #wiki 内容\n- 其他内容",
            ...
            {"name": "多个标签", "content": "- #算法 #wiki #计算机科学\n- 内容", "expected": "Wiki"},
            {
                "name": "非匹配标签",
        ...
        for case in test_cases:
            with self.subTest(case=case["name"]):
                data = {"content": case["content"], "meta_properties": []}
                ...
                self.assertEqual(
                    category, case["expected"], f"案例 '{case['name']}' 检测结果错误"
                )
    ...
    def test_bullet_removal_logic(self):
        ...
        for input_line, expected in test_cases:
            with self.subTest(input_line=input_line):
                ...
                self.assertEqual(result, expected, f"'{input_line}' 应该转换为 '{expected}'")
