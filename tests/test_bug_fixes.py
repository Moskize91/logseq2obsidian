...
class TestBugFixes(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.formatter_without_bullets = ObsidianFormatter(remove_top_level_bullets=True)
...
- 另一个问题'''.split('\n')
        ...
        result_lines = self.formatter_without_bullets._remove_top_level_bullets(test_content)
        result_text = '\n'.join(result_lines)
...
\t- 阻力越大，替代物离潜意识本意越远。'''
        ...
        # 使用完整的格式化流程测试缩进规范化
        result = self.formatter_without_bullets.format_content({'content': test_content})
        ...
        # 验证一级子列表使用 2 个空格缩进
        for line in result_lines:
            if line.strip().startswith('- ') and line.startswith('  ') and not line.startswith('    '):
                indent = len(line) - len(line.lstrip())
                expected_indent = 2  # 一级缩进
                self.assertEqual(indent, expected_indent, 
...
- 另一个问题'''
        ...
        # 验证块引用被注释
        self.assertIn("<!-- Block Reference: 6211a9d9-f200-4348-a308-23381ba25cb9 -->", result)
