...
""",
        ...
        self.assertIn(
            "<!-- Block Embed (未找到): unknown-uuid -->", result, "未知UUID的embed应该生成注释"
        )
        ...
        self.assertEqual(
            self.formatter.referenced_uuids,
            expected_uuids,
...
""",
            ...
            # 验证embed转换
            self.assertIn(
                "![[target_page#^block1]]", main_result, "Embed应该正确转换为Obsidian格式"
            )

            # 验证块引用转换（这里应该是注释，因为main-block-id在同一阶段未被处理）
            self.assertIn(
                "<!-- Block Reference (未找到): main-block-id -->",
                target_result,
                "未找到的块引用应该生成注释",
            )
    ...
    def test_embed_edge_cases(self):
        ...
        # 测试只有embed的内容
        test_data = {"content": "{{embed ((test-uuid))}}", "meta_properties": []}
