...
# 真实的 Logseq 目录路径
LOGSEQ_SOURCE_DIR = Path(
    "/Users/taozeyu/Library/Mobile Documents/iCloud~com~logseq~logseq/Documents"
)
# 测试输出目录（会被清空）
...
def test_sample_files(max_files=5):
    ...
    for i, md_file in enumerate(test_files, 1):
        ...
        try:
            ...
            print(f"   ✅ 转换成功")
            print(
                f"   📊 页面链接: {conversion_summary['original']['page_links']} → {conversion_summary['converted']['page_links']}"
            )
            print(
                f"   📊 块引用: {conversion_summary['original']['block_refs']} → {conversion_summary['converted']['block_refs']} (注释)"
            )
            print(
                f"   📊 块ID: {conversion_summary['original']['block_ids']} → {conversion_summary['converted']['block_ids']}"
            )
